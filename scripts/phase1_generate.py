...
if __name__ == "__main__":
    ...
    if api_provider == "google":
        api_settings = config.get("google_settings", {})
        if not api_settings:
            print("Error: 'google_settings' not found in config.yaml")
            sys.exit(1)
        if not configure_google_genai(api_settings):
            print("Exiting due to Google GenAI configuration failure.")
            sys.exit(1)
                ...
                for model_name in models_to_use:
                    ...
                    if api_provider == "google":
                        raw_response, prompt_tokens, completion_tokens = (
                            call_google_genai_llm(model_name, full_prompt, api_settings)
                        )
                    elif api_provider == "openrouter":
                        raw_response, prompt_tokens, completion_tokens = (
                            call_openrouter_llm(
